...
class R2PhotoDownloadUrlGeneratorTest(
    private val generator: PhotoDownloadUrlGenerator,
    private val clock: Clock,
) : BehaviorSpec({

    given("사진 오브젝트 키가 있으면") {
        `when`("R2 다운로드 URL을 생성할 때") {
            ...
            val result = generator.generate(objectKey)
            val issuedAtLatest = LocalDateTime.now(clock)
            val uri = URI(result.downloadUrl)
            val queryParameters = uri.downloadQueryParameters()

            then("test 버킷의 오브젝트 키로 GET URL을 서명한다") {
                ...
                queryParameters.getValue("X-Amz-SignedHeaders") shouldBe "host"
            }
...
private fun URI.downloadQueryParameters(): Map<String, String> =
    rawQuery
        .split("&")
        .associate { parameter ->
