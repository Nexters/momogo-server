...
class UserTest : BehaviorSpec({
    ...
    given("유효한 회원 정보가 있으면") {
        ...
        `when`("길이가 1자 또는 12자인 닉네임을 사용할 때") {
            then("경계 길이의 닉네임을 허용한다") {
                User(_nickname = "가").nickname shouldBe "가"
                User(_nickname = "가".repeat(12)).nickname shouldBe "가".repeat(12)
            }
        }
    }
    ...
    given("유효하지 않은 닉네임이 있으면") {
        ...
        `when`("12자를 초과한 닉네임으로 회원을 생성할 때") {
            then("생성을 거부한다") {
                shouldThrow<IllegalArgumentException> {
                    User(_nickname = "가".repeat(13))
                }.message shouldBe "닉네임은 12자를 초과할 수 없습니다."
            }
        }
        ...
        `when`("유효하지 않은 닉네임으로 변경할 때") {
            then("기존 닉네임을 유지한다") {
                ...
                listOf("   ", "가".repeat(13)).forEach { nickname ->
                    shouldThrow<IllegalArgumentException> {
